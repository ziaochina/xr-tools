...
function buildIndex(appFolder) {
	...
	*/
	var importAppsContent = apps.map(o => `import ${o.name} from '${path.relative(path.join(o.path,'index.js'), basePath)}'`).join('\r\n')
...
Object.keys(xrComponents).forEach(key=>{
	...
	var appLessContent = apps.map(
		o => `@import "${path.relative(path.join(o.path, 'style.less'),path.join(basePath,'assets','styles','app.less'))}"`).join('\r\n')
