...
function compile() {
	...
	var indexTemplate = _fs2.default.readFileSync(_path2.default.join(basePath, 'index.template'), 'utf-8');

	var indexContent = indexTemplate.replace('${import-apps}', importAppsContent).replace('${config-apps}', configAppsContent).replace('${root-app-name}', rootAppName);
