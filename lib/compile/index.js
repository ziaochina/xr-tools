...
function buildIndex(appFolder) {
	...
	//获取文件数组
	var findApps = function findApps(absoultePath, relaPath) {
		...
		files.forEach(function (filename) {
			...
			//是文件
				...
				findApps(_path2.default.join(absoultePath, filename), relaPath + '/' + dirName);
			}
		});
	};
 ...
 */
		...
		return 'import ' + o.name + ' from \'' + o.path + '\'';
	}).join('\r\n'
...
function copy(appFolder, cb) {
	...
	var dest = join(process.cwd(), 'src', 'apps', _path2.default.basename(cwd));
